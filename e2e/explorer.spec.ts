...
test.describe("Explorer page", () => {
  ...
  test("destination routes work — each slug resolves to a valid fiche", async ({ page }) => {
    ...
    const slugs = [
      ...
      "pekin", "osaka", "busan", "fukuoka", "sapporo", "bologne", "berlin", "londres", "rome", "madrid", "milan", "venise", "bruxelles", "dublin", "cork", "valence", "hambourg", "munich",
    ];
  ...
  test("fallback grid: 67 cards present and linked to /destination/[slug]", async ({ page }) => {
    await page.goto("/explorer");
