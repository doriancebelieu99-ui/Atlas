...
describe("getItineraryBySlug", () => {
  ...
  it("returns itinerary for Cork (4 days)", () => {
    const result = getItineraryBySlug("cork");
    expect(result).not.toBeNull();
    expect(result!.days).toHaveLength(4);
  });

  it("returns itinerary for Valence (5 days)", () => {
    const result = getItineraryBySlug("valence");
    expect(result).not.toBeNull();
    expect(result!.days).toHaveLength(5);
  });

  it("returns itinerary for Hambourg (5 days)", () => {
    const result = getItineraryBySlug("hambourg");
    expect(result).not.toBeNull();
    expect(result!.days).toHaveLength(5);
  });

  it("returns itinerary for Munich (5 days)", () => {
    const result = getItineraryBySlug("munich");
    expect(result).not.toBeNull();
    expect(result!.days).toHaveLength(5);
  });

  it("malaga — 5 jours", () => {
    const result = getItineraryBySlug("malaga");
    expect(result).not.toBeNull();
    expect(result!.days).toHaveLength(5);
  });
